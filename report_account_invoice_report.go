...
func init() {
	...
	h.AccountInvoiceReport().DeclareManualModel()
	...
	h.AccountInvoiceReport().AddFields(map[string]models.FieldDefinition{
		...
		"UserCurrencyPriceTotal": models.FloatField{String: "Total Without Tax",
			...
			Digits:  nbutils.Digits{Precision: 16}},
...
SELECT sub.id, sub.date, sub.product_id, sub.partner_id, sub.country_id, sub.account_analytic_id,
	...
	sub.category_id, sub.date_due, sub.account_id, sub.account_line_id, sub.partner_bank_id,
...
SELECT ail.id AS id,
	...
	1 AS nbr,
	ai.type, ai.state, pt.category_id, ai.date_due, ai.account_id, ail.account_id AS account_line_id,
	ai.partner_bank_id,
...
JOIN partner ON ai.partner_id = partner.id
LEFT JOIN product_product pr ON pr.id = ail.product_id
left JOIN product_template pt ON pt.id = pr.product_tmpl_id
LEFT JOIN product_uom u ON u.id = ail.uom_id
LEFT JOIN product_uom u2 ON u2.id = pt.uom_id
LEFT JOIN product_uom_hexya_name un ON u2.id = un.record_id AND un.lang = ''
JOIN (
...
GROUP BY ail.id, ail.product_id, ail.account_analytic_id, ai.date_invoice, ai.id,
	ai.partner_id, ai.payment_term_id, un.name, u2.id, ai.currency_id, ai.journal_id,
	ai.fiscal_position_id, ai.user_id, ai.company_id, ai.type, invoice_type.sign, ai.state, pt.category_id,
	ai.date_due, ai.account_id, ail.account_id, ai.partner_bank_id, ai.residual_company_signed,
	ai.amount_total_company_signed, partner.commercial_partner_id, partner.country_id
`
			return groupByStr
		})
...
CREATE or REPLACE VIEW %s as (
	...
	LEFT JOIN currency_rate cr ON
		...
		(cr.date_end IS NULL OR cr.date_end > COALESCE(sub.date, NOW())))
				...
				h.AccountInvoiceReport().TableName(),
				h.AccountInvoiceReport().TableName(),
